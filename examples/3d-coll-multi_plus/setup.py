...
shape1=shape_dict.plus3d(l=lx, meshsize=meshsize)

material = material_dict.peridem_3d(delta)

# material = material_dict.kalthoff3d(delta)

SL.append(shape=shape1, count=7, meshsize=meshsize, material=material)
...
cnst = contact_radius*1.1

#  (x,y,z)---> y = movehorizontally, z = moving vertically
#apply transformation
particles[0][0].rotate3d('z', -np.pi/4)
particles[0][6].rotate3d('x', -np.pi/4)

particles[0][0].shift([0, 0.3*rad+cnst,                     0])
particles[0][1].shift([0, 0           , 2*rad+cnst])
particles[0][2].shift([0, 4*rad+cnst  , 4*rad+cnst])
particles[0][3].shift([1*rad+cnst,-1*rad+cnst,    3.0*rad+cnst])
particles[0][4].shift([0,             2*rad+cnst, 2.5*rad+cnst])
particles[0][5].shift([1*rad+cnst, -1.5*rad+cnst, -2.0*rad+cnst])
particles[0][6].shift([0,            2*rad+cnst, -2.5*rad+cnst])
...
particles[0][2].vel += [0, 0, args.vel_val]
particles[0][3].vel += [0, 0, args.vel_val]
