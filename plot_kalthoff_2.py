...
if args.all_dir:
    ...
    setup_filename = f'{args.all_dir}/setup.h5'

# Plot info
plotinfo_file = f'{data_dir}/plotinfo.h5'
with h5py.File(plotinfo_file, "r") as p:
...
def total_neighbors(conn, N):
    ...
    :returns: Total number of neighbors for each node
    """
    deg = np.zeros(N)
...
# Ref connectivity
with h5py.File(setup_filename, "r") as f:
    ref_n = {}
    for name in f:
        if re.match(r'P_[0-9]+', name):
            ref_connectivity = np.array(f[f'{name}/Connectivity']).astype(int)
            N = len(f[f'{name}/Pos'])
...
def genplot(t):
    print(t)
    tc_ind = f'{t:05d}'
    filename = f'{data_dir}/tc_{tc_ind}.h5'
    wall_filename = f'{data_dir}/wall_{tc_ind}.h5'
    out_png = f'{img_dir}/{args.img_prefix}_{tc_ind}.png'
    ...
    with h5py.File(filename, "r") as f:
        for name in f:
            if re.match(r'P_[0-9]+', name):
                pid = int(name[2:])
                Pos = np.array(f[f'{name}/CurrPos'])
                c = 'blue'
                vmin, vmax = None, None

                if args.quantity == 'damage':
                    P_conn = np.array(f[f'{name}/Connectivity']).astype(int)
                    N = len(Pos)
                ...
                dval = scaled_ds[pid] if args.adaptive_dotsize else dotsize

                forcolorbar = ax.scatter(
                    Pos[:, 0], Pos[:, 1], Pos[:, 2], c=c, s=dval,
                    marker='.', linewidth=0, cmap=args.colormap,
                    vmin=vmin, vmax=vmax, alpha=args.alpha
                )

                if args.plot_contact_force and pid == args.wheel_ind:
                    cnode_list = []
                    nbr_name = f'P_{args.nbr_ind:05d}'
                    nbr_currpos = np.array(f[f'{nbr_name}/CurrPos'])
                    for node in range(len(Pos)):
                        for i in range(len(nbr_currpos)):
                            ...
                            if dist <= contact_radius:
                                ...
                                break
                    ax.scatter(
                        Pos[cnode_list, 0], Pos[cnode_list, 1], Pos[cnode_list, 2],
                        color='red', s=dval, marker='.', linewidth=0, cmap=args.colormap
                    )

                if args.plot_contact_rad and pid == args.contact_rad_ind:
    ...
    if not args.nowall:
        ...
        with h5py.File(wall_filename, "r") as w:
    ...
    if not args.nogrid:
        plt.grid()
    if args.xlim:
        xx = list(map(float, args.xlim.split(' ')))
        ax.set_xlim3d(xx)
    if args.ylim:
        yy = list(map(float, args.ylim.split(' ')))
        ax.set_ylim3d(yy)
    if args.zlim:
        zz = list(map(float, args.zlim.split(' ')))
        ax.set_zlim3d(zz)

    mx = np.amax(np.abs(ss))
    ax.set_xlim3d([ 0., mx - 0.1])
    ax.set_ylim3d([-0.05,  0.1])
    ax.set_zlim3d([0., mx - 0.02])
