...
#args.highlight_region = [x1m, x2m, y1m, y2m, z1m, z2m]
args.highlight_region = [0.0,0.04,0.0,0.04,-0.04,0.04]
#args.highlight_region = [-0.00,0.02,-0.00,0.02,0.0,0.04]
def genplot(t):
    print(t)
    tc_ind = ('%05d' % t)
    ...
    for name in f:
        if re.match(r'P_[0-9]+', name):
            ...
            forcolorbar = ax.scatter(Pos[:, 0], Pos[:, 1], Pos[:, 2], c='gray', s=dval, marker='.', linewidth=0, cmap=args.colormap, vmin=vmin, vmax=vmax, alpha=0.1)
